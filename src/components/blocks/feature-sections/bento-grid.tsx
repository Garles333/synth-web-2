  ...
  return (
    <div className="relative z-20 mx-auto max-w-7xl py-10 lg:py-40">
      ...
      <div className="relative mt-12">
        {/* Imagen de fondo más viva y a sangre lateral */}
        <div className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
          <img 
            ...
            alt="Arquitectura Tecnológica Background"
            className="w-full h-full object-cover object-center"
            style={{ 
              opacity: .94, // mucho más color
              filter: 'saturate(1.35) brightness(1.18) blur(0.2px)' 
            }}
          />
          {/* Overlay solo para resaltar legibilidad, muy suave */}
          <div 
            className="absolute inset-0"
            style={{
              background: 'linear-gradient(180deg,rgba(11,14,26,0.52) 0%,rgba(11,14,26,0.77) 100%)',
            }}
          />
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-6 relative rounded-none border-none bg-transparent p-0 w-full">
...
// Animación tipo "puzzle encaje" en cards
const puzzleConfigs = [
  // Variantes de entrada (x/y/rotate) por índice de card, tipo puzzle
  { x: -60, y: -30, rotate: -4 }, // Card 0
  { x: 60, y: 30, rotate: 6 }, // Card 1
  { x: -70, y: 20, rotate: -5 }, // Card 2
  { x: 50, y: 50, rotate: 7 } // Card 3
];

// Wrapper con animación puzzle
const AnimatedFeatureCard = ({ children, className, index }: { children: React.ReactNode; className?: string; index: number }) => {
  const ref = useRef<HTMLDivElement>(null);
  const isInView = useInView(ref, {once: true, amount: 0.3});
  const config = puzzleConfigs[index % puzzleConfigs.length];
  return (
    <motion.div
      ref={ref}
      initial={{
      ...
      className={cn("relative overflow-hidden p-4 sm:p-8 bg-[rgba(26,31,46,0.96)] backdrop-blur-md rounded-lg border border-[var(--color-border)] shadow-none", className)}
      style={{ minHeight: 200 }}
    >
      {children}
    </motion.div>
